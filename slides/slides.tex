...
\begin{frame}{Heuristic derivation}
  ...
  \[
    c'(a_0)
    \geq \bigl[\ell(\lambda)-u(w_0)\bigr]
...
\begin{frame}{Heuristic derivation}
  ...
  \[
    U_{aa}
    \leq\int_{\{s>q_1\}}
      \bigl[\ell(\lambda+\mu s)-\ell(\lambda)\bigr]
      f_{aa}\,\mathrm{d}y
      -c''(a).
  \]
...
\begin{frame}{Heuristic derivation}
  With $\rho:=\lim_{z\to\infty}z\ell_0'(z)$,
  \begin{eqnarray*}
    U_{aa}
    &\leq&
    ...
    &\to&
    \dfrac{\rho\,\mathrm{Curv}(q_1)}{q_1-q_0}-\underline c''.
  \end{eqnarray*}
\end{frame}
