...
Section \ref{sec:examples} explains our results through simple examples and reconciles our findings with the literature. Section \ref{sec:model} gives definitions. Section \ref{sec:main-result} states Theorem \ref{thm:main}. Section \ref{sec:main-result-proof} outlines the proof. Section \ref{sec:calculus} provides a broad range of closed-form solutions for optimal contracts. Section \ref{sec:linear-contracts} shows that optimal contracts are piecewise linear option contracts for log utility and output distributions in an exponential family with linear sufficient statistic (this has been independently discovered by \textcite{opp2025moral}). Section \ref{sec:numerical-methods} provides numerical methods for both the case where the FOA is valid and in the more general case where it is not. Section \ref{sec:counter-examples} gives counter-examples and discusses limitations of our results. Section \ref{sec:literature-discussion} discusses the relationship between our results and the literature.