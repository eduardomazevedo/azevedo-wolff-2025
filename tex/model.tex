...
A risk-neutral \textbf{principal} hires an \textbf{agent} with limited liability. The agent has utility $u(x)-c(a)$ from receiving a \textbf{payment} $x \in \mathbb{R}_{+}$ and taking \textbf{action} $a$ in $\mathcal A \subseteq \mathbb{R}_+$. Both $u$ and $c$ are strictly increasing. \textbf{Output} $y \in \mathbb{R}$ depends on the agent's action and is distributed according to the \textbf{output density} $f(y|a)$.
...
For an interior intended action, the \textbf{relaxed cost minimization problem} replaces the global incentive compatibility constraint with the local incentive compatibility constraint (\ref{LIC_relaxed}). For a boundary intended action, the corresponding local incentive condition would be an inequality rather than the equality below.
\begin{align}
    \text{minimize} \quad & W(v, a_0) \nonumber \\
...
We now state the needed assumptions. This section can be skimmed on a first reading.
...
\begin{enumerate}
    ...
    \[
        \left|v(y) \partial^n_a f(y|a)\right| \leq \theta(y)
    \]
...
We now consider substantive restrictions on the distribution of output.%
\footnote{The most unusual assumption is the third one. It holds for example in Gaussian, logistic, and Student-$t$ location families. Examples that do not satisfy the assumption can be constructed with families of distributions where this second derivative oscillates in the left tail.}
...
\begin{assumption}[Assumptions on the distribution of output]
    ...
    \begin{enumerate}
        ...
        \item For all $a$, the score $\partial_a \log f(y|a)$ is strictly increasing in $y$ (the standard monotone likelihood ratio property, or MLRP) and its image is $\mathbb R$.
        \item There exists $y_0$ in $\mathbb R$ such that, for all $y \leq y_0$ and $a$ in $\mathcal{A}$,
        $$
        f_{aa}(y|a) > 0 \text{.}
        $$
    \end{enumerate}
...
Another substantial restriction is sufficient concavity of $k'^{-1}$.
...
The function $k'^{-1}$ is the same as \citeauthor{jewitt1988justifying}'s (\citeyear{jewitt1988justifying}) $\omega(z)$. So our assumption that $k'^{-1}$ is strictly concave corresponds to his assumption (2.12). The key difference is that \textcite{jewitt1988justifying} does not have a limited liability constraint and requires that the first-order formula for an optimal contract holds globally. This requirement often implies that there is no solution to the principal's problem. For example, in the Gaussian-log utility example, there is no solution under Jewitt's assumptions (see derivation in Section \ref{sec:main-result-proof}). \textcite{jewitt1988justifying} also requires concavity of the score function to guarantee that the optimal contract is concave in $y$. We do not require this.