...
\begin{proof}
    ...
    Observe that $u(0) - g(\lambda) \rightarrow -\infty$, and $ F_a(\ubar y (\lambda) | a_0) < 0 $ by Assumption \ref{assump:regularity_S}.2. Supp ose $F_a(\ubar y (\lambda) | a_0)$ does not approach $0$. Then we have $c'(a_0) \geq \infty$. By contradiction, $F_a(\ubar y (\lambda) | a_0) \rightarrow 0$. 

    The threshold outcome, $\ubar y (\lambda)$, cannot converge to positive infinity because $\ubar S(\lambda)$ is negative for $\lambda$ sufficiently large because $S(\infty | a_0) = \infty$ by Assumption \ref{assump:regularity_S}.3, and $\infty > \ubar S(\lambda)$. If $\ubar S (\lambda)$ does not converge to negative infinity, then there is a subsequence that converges to a finite number. However, $F_a(c | a_0) < 0$ for any $c$ by Assumption \ref{assump:regularity_S}.2. The proposition is proven by contradiction. 
\end{proof}    
