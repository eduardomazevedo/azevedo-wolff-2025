...
We first note that there is no upper bound on the utility levels attained by the LIC-satisfying canonical contracts.
...
$\lambda ^*$ strictly increasing and $\lim_{\bar U\rightarrow\infty}\lambda^*(\bar U)=\infty$ follow from Lemma \ref{u-tilde-increasing} and Lemma \ref{lem:utilde-unbounded}. The strict convexity of $\omega$ follows from $v^*(\cdot | \lambda)$ being different for each value of $\lambda$ and from $k$ being strictly convex.
