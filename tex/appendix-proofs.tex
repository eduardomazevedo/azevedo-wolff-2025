...
Throughout this subsection, fix a reservation utility $\bar U$ and regular intended action $a_0$. The relaxed problem only has a solution if $U(v, a)$ can be differentiated under the integral sign, so we begin by showing that differentiation under the integral at any action is possible for any feasible contract.

\begin{remark} 
    \label{rem:leibniz}
    Let $v$ be a feasible contract. Then, for all $a$ in $\mathbb{R}_{+}$, $U_{a}(v,a)$ exists, is finite, and
    \[
...
$\bar{U}_R$ equals $u(\infty) - c(a_0)$, so the relaxed problem is not feasible for $\bar{U} \geq \bar{U}_R$, as desired.
...
Let $\mu ^* (\lambda) := \tilde \mu (\lambda ^* (\bar U))$. Lemma \ref{lem:mu-tilde} implies that $\mu^*(\bar U) > 0$. Let $v^*(y | \bar U) := \tilde V (y | \lambda ^* (\bar U))$. Note that this coincides with the definition of $v^*$ in the proposition statement.

Lemma \ref{lem:pareto-problem} implies that $v^*(y | \bar U)$ solves the Pareto problem given $\lambda^*(\bar U)$. This implies that $v^*(y | \bar U)$ also solves the relaxed cost minimization problem. Likewise, \ref{lem:pareto-problem} implies that the solution is unique almost everywhere.
...
\begin{proof}
The expression for $U(v,a)$ is just the definition. 
