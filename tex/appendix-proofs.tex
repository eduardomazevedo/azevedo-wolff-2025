...
\[
    ...
    =
    \int [k(v(y)) - \lambda v(y) - \mu v(y)S(y|a_0)]f(y|a_0)\, dy + \lambda\left(\bar U + c(a_0)\right) + \mu c'(a_0)
    \text{.}
\]
...
Therefore, $\tilde U$ is non-decreasing. It only remains to show that $\tilde U$ is strictly increasing. To reach a contradiction, assume that $\tilde U(\lambda _ 2) = \tilde U (\lambda _ 1)$. Optimality implies that $\tilde W (\lambda _1) = \tilde W (\lambda _2)$.  Consider the contract $(v_1 + v_2)/2$. By our assumptions on the score, $v_1$ and $v_2$ differ in a set of positive measure. By strict convexity of $k$, it follows that $W((v_1 + v_2)/2) < W(v_1)$. This contradicts the optimality of $v_1$.

Continuity follows from the continuity of the canonical contract in $(\lambda,\mu)$ and Assumption \ref{assump:leibniz}. To see that $\tilde\mu(\lambda)$ is continuous, define
\[
    H(\lambda,\mu)
    :=
    \int V(y|\lambda,\mu)f_a(y|a_0)\,dy - c'(a_0).
\]
The function $H$ is continuous and, by the strict monotonicity argument in Lemma \ref{lem:mu-tilde}, strictly increasing in $\mu$ at the solution to $H(\lambda,\mu)=0$. Hence the unique solution $\tilde\mu(\lambda)$ varies continuously with $\lambda$, and so does $\tilde U(\lambda)$.
\end{proof}

We next note that there is no upper bound on the utility levels attained by the LIC-satisfying canonical contracts.

\begin{lemma}
    \label{lem:utilde-unbounded}
    $\lim _ {\lambda \rightarrow \infty} \tilde U(\lambda) = \infty$.
\end{lemma}

\begin{proof}
Let $Y_+ := \{y:S(y|a_0) \geq 0\}$. Assumption \ref{assump:regularity_S} implies that $Y_+$ has positive probability. Since $g$ is increasing, for $y \in Y_+$,
\[
    \tilde V(y|\lambda)
    = g(\lambda + \tilde \mu(\lambda)S(y|a_0))
    \geq g(\lambda).
\]
For all $y$, $\tilde V(y|\lambda) \geq u(0)$. Therefore,
\[
    \tilde U(\lambda)
    \geq
    \Pr(Y_+)g(\lambda)
    ...
    - c(a_0).
\]
Assumption \ref{assump:utility_cost} implies that $u$ is unbounded above and $u'(x)\rightarrow 0$, so $g(\lambda)\rightarrow\infty$. Hence $\tilde U(\lambda)\rightarrow\infty$.
\end{proof}
...
Let $\bar U _ L : = \tilde U (0)$.

\textbf{Part 1: existence, uniqueness, and characterization.}

The relaxed problem is feasible for every reservation utility $\bar U$. If $\bar U \leq \bar U_L$, feasibility follows from the contract $\tilde V(\cdot|0)$. If $\bar U > \bar U_L$, Lemma \ref{u-tilde-increasing} and Lemma \ref{lem:utilde-unbounded} imply that there exists $\lambda>0$ such that $\tilde U(\lambda) = \bar U$, and $\tilde V(\cdot|\lambda)$ is feasible.

Let $\lambda ^*(\bar U)$ be $0$ if $\bar U \leq \bar U _L$ and be the inverse of $\tilde U$ for $\bar U _L < \bar U$. Lemma \ref{u-tilde-increasing} and Lemma \ref{lem:utilde-unbounded} imply that $\lambda^*$ is well defined and that $\lambda^*(\bar U) \geq 0$.
...
Lemma \ref{lem:pareto-problem} implies that $v^*(y | \bar U)$ solves the Pareto problem given $\lambda^*(\bar U)$. This implies that $v^*(y | \bar U)$ also solves the relaxed cost minimization problem given $\bar U$. Likewise, Lemma \ref{lem:pareto-problem} implies that the solution is unique almost everywhere.

\textbf{Part 2: comparative statics.}

For $\bar U \leq \bar U_L$, the definition gives $\lambda^*(\bar U)=0$, so the relaxed optimal contract and optimal expected wage do not vary with $\bar U$.

For $\bar U > \bar U_L$, $\lambda ^*$ is strictly increasing and $\lim_{\bar U\rightarrow\infty}\lambda^*(\bar U)=\infty$ by Lemma \ref{u-tilde-increasing} and Lemma \ref{lem:utilde-unbounded}. The strict convexity of $\omega$ follows from $v^*(\cdot | \lambda)$ being different for each value of $\lambda$ and from $k$ being strictly convex.
...
\begin{proof}
    The utility function, \( U \), evaluated at the canonical contract, $\tilde{V}\left( y | \lambda \right)$, and action, $a$, is
    \[
...
\begin{definition}
    ...
    A score that satisfies the equation exists by Assumption \ref{assump:regularity_S}.2, which states that the score's image is $\mathbb{R}$. 
\end{definition} 
...
\begin{proof}
    Lemma \ref{lem:threshold-outcome} and assumption \ref{assump:regularity_S}.2 imply that $\ubar S (\lambda) \rightarrow -\infty$. The result follows from the definition of $\ubar S (\lambda)$.  
\end{proof}
...
By Lemma \ref{lem:second-derivative}, 
    ...
    =
    \tilde{\mu} \int \Delta g(y | \lambda)
    \cdot S(y|a_0)
    \cdot f_{aa}(y | a) \, dy - c''(a)
    $$ 
    Assumption \ref{assump:regularity_S}.3 implies that there exists $y_0$ such that $f_{aa}(y | a) > 0$ for all $y \leq y_0$ and $a$. Because the score is monotone, we can take $y_0$ such that $S(y|a_0) < 0$ for all $y \leq y_0$. Therefore, the integrand is negative for all $y \leq y_0$ and $a$.    
    ...
    Let $Y_{aa}^+$ be the set of $y \geq y_0$ where this integrand is positive. Then 
    $$
    U_{aa}\left( \tilde{V} \left( y |  \lambda \right), a \right) \leq
    \tilde{\mu} \int_{Y_{aa}^+} 
    \Delta g(y | \lambda)
    ...
    $$
     ...
     Lemma \ref{lem:threshold-outcome} implies that for $\lambda$ sufficiently large, $\ubar y (\lambda, a_0) \leq y_0$. To see that $\Delta g$ is decreasing for $y$ in $Y_{aa}^+$, note that on the nonbinding region,
    \[
        \Delta g(y|\lambda)
        =
        \frac{g(z(y))-g(\lambda)}{z(y)-\lambda},
        \quad
        z(y):=\lambda+\tilde\mu S(y|a_0).
    \]
    By the concavity of $k'^{-1}$ (Assumption \ref{assump:concavity_inverse_marginal_utility}), $g$ is concave on the nonbinding region, so this secant slope is decreasing in the endpoint $z(y)$. Since $\tilde\mu>0$ and the score is increasing, $z(y)$ is increasing in $y$. Thus $\Delta g$ is decreasing in $y$. This argument does not require concavity of the score. Therefore,
    $$
    U_{aa}\left( \tilde{V} \left( y |  \lambda \right), a \right) \leq
    \tilde{\mu} \int_{Y_{aa}^+} 
    ...
    The link function $g$ is concave in its scalar argument on the nonbinding region. Thus,
    \begin{align*}
...
\begin{proof}[Proof of Proposition \ref{prop:concave}]
    ...
    To reach a contradiction, assume that this is not the case. Then there exists a sequence of $\lambda_n \rightarrow \infty$ and $a_n$ in $\mathcal A$ such that
    \[
        U_{aa} \left(\tilde V(y | \lambda_n), a_n \right) > 0\text{.}
    \]
    
    Because $\mathcal A$ is compact, we can take a convergent subsequence where $a_n \rightarrow a_1$. Continuity of $U_{aa}(\tilde V(y | \lambda),a)$ in $a$, which follows from Assumptions \ref{assump:utility_cost} and \ref{assump:leibniz}, gives
    \[
