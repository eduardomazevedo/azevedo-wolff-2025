...
Throughout this subsection, fix a reservation utility $\bar U$ and intended action $a_0$. Let $Y_+ := \{y:S(y|a_0) \geq 0\}$ denote the positive-score region. The local IC constraint is only defined if $U(v, a)$ can be differentiated under the integral sign, so we begin by showing that differentiation under the integral at any action is possible for any feasible contract.
...
\begin{proof}
Assumption \ref{assump:regularity_S} implies that $Y_+$ has positive probability. Since $g$ is increasing, for $y \in Y_+$,
\[
    \tilde V(y|\lambda)
...
By Lemma \ref{lem:second-derivative}, 
    ...
    $$
     ...
     Lemma \ref{lem:threshold-outcome} implies that for $\lambda$ sufficiently large, $\ubar y (\lambda, a_0) \leq y_0$. The concavity of $k'^{-1}$ (Assumption \ref{assump:concavity_inverse_marginal_utility}) implies that $\Delta g$ is decreasing for $y$ in $Y_{aa}^+$. Therefore,
    $$
    U_{aa}\left( \tilde{V} \left( y |  \lambda \right), a \right) \leq
    \tilde{\mu} \int_{Y_{aa}^+} 
