We can now state our main result. We say that the first-order approach is \textbf{valid} for an action and reservation utility pair $(a_0, \bar U)$ if any solution to the relaxed cost minimization problem is a solution to the cost minimization problem.
...
In particular, the theorem implies that the cost minimization problem has a tractable solution, given by the standard formulae in the first-order approach literature. We establish the solution in Proposition \ref{prop:relaxed-optimal-contract} below, and note the implication to the full problem here.
