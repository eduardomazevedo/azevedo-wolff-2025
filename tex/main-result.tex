...
\begin{theorem}
    ...
    Given an interior action $a_0 > 0$, the first-order approach is valid for $(a_0, \bar U)$ for any sufficiently high reservation utility $\bar U$.
\end{theorem}
...
\begin{corollary}
    ...
    Given an interior action $a_0 > 0$, for any sufficiently high reservation utility $\bar U$, the cost minimization problem has an almost everywhere unique solution given by the formula in Proposition \ref{prop:relaxed-optimal-contract}.
\end{corollary}