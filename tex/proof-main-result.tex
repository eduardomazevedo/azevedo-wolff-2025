...
In our case of limited liability, the solution is described with the following notation. Define the {\bf optimal expected wage} $\omega(\bar U)$ as the value of the infimum in the relaxed cost minimization problem. Define the \textbf{link function} $g:\mathbb R \rightarrow \mathbb R$ as\footnote{%
...
\begin{figure}[p]
    \centering
    \includegraphics{figures/log-gaussian-sigma=50.0/cm_stacked.pdf}
    \caption{Optimal contracts for cost minimization problem with $a_0 = 100K$, Gaussian distribution, and log utility.}
...
\begin{figure}[ht]
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \texttt{FIGURE HERE: norm\_log\_probs.pdf}
        \caption{Probability of payment}
        \label{fig:norm_log_probs}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.45\textwidth}
        \texttt{FIGURE HERE: norm\_log\_exp\_wage.pdf}
        \caption{Expected wage}
        \label{fig:norm_log_exp_wage}
    \end{subfigure}
    \caption{Probability of payment and expected wages in the Gaussian-log utility example}
    \captionsetup{font=footnotesize} % Makes the note footnote-sized
