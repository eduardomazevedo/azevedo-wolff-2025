...
We first show that the relaxed cost minimization problem has an almost everywhere unique solution, and that this solution has a simple formula in terms of Lagrange multipliers. The solution is based on the first order condition, and is similar to standard formulas in the literature, such as in the case without limited liability.\footnote{%
...
\begin{proposition}
    ...
    \begin{enumerate}
        \item (infeasible region) For $\bar U \geq \bar U _ R$, the relaxed problem is not feasible.
        \item (feasible region) For $\bar U < \bar U _R$, the relaxed problem has an almost everywhere unique solution $v^*(y|\bar U)$. There exist Lagrange multipliers $\lambda^*(\bar U) \geq 0$ and $\mu^*(\bar U) > 0$ such that the solution is almost everywhere equal to the canonical contract
    \[
        \label{eq:relaxed-optimal-contract}
        v^*(y|\bar U) := V\left(y | \lambda^*(\bar U), \mu^*(\bar U)\right)
        \text{.}
    \]
        \item (comparative statics) In the feasible region,
        \begin{itemize}
...
Part (1) covers the case when $\bar U$ is so high that the relaxed problem is not feasible. For example, if $u(\cdot)$ is bounded and $u(\infty) - c(a_0)$ is smaller than $\bar U$, the relaxed problem is not feasible. As we see below, in many examples, the relaxed problem is feasible for all $\bar U$ in $\mathbb R$, so that $\bar U _R = \infty$ and part (1) is vacuous. Part (2) is the core of the result. It says that, whenever the relaxed problem is feasible, the solution is a canonical contract. 
...
\]
This is the piecewise linear solution that we saw numerically in figure \ref{fig:norm_log_con_opt}. The convex Pareto frontier is illustrated in figure \ref{fig:pf}. The relaxed problem is always feasible, so the infeasible region is empty ($\bar U_R = \infty$).

The figure illustrate the comparative statics from part (3). For sufficiently low $\bar U$, the (\ref{IR}) constraint is not binding, and thus the solution does not depend on $\bar U$. In the region where the (\ref{IR}) constraint is binding, expected wages and the Lagrange multiplier $\lambda^*(\bar U)$ are strictly increasing in $\bar U$.
...
\begin{proposition} 
    ...
    There exists $U^* < \bar{U}_R \in \mathbb{R}$ such that, for all $\bar U$ with $U^* < \bar{U} < \bar U _ R$, and all $a \in \mathcal{A}$,  
    $$ U_{aa}(v^* (y | \bar{U}), a) \leq 0 .$$ 
\end{proposition}

Theorem \ref{thm:main} follows from proposition \ref{prop:concave} and proposition \ref{prop:relaxed-optimal-contract}:

\begin{proof}[Proof of Theorem \ref{thm:main}]
    Take $\bar U \geq U ^*$. Proposition \ref{prop:relaxed-optimal-contract} implies that there are two cases. If $\bar U \geq \bar U _R$, neither the relaxed cost minimzation problem nor the cost minimization problem are feasible.
    
    If $\bar U < \bar U _R$, then the relaxed cost minimization problem has a solution $v^*(y | \bar U)$. Proposition \ref{prop:concave} implies that the agent's expected utility $U(v^*,a)$ is concave in $a$. Therefore, the global incentive compatibility constraint is satisfied, and the solution to the relaxed cost minimization problem is also a solution to the cost minimization problem.
\end{proof} 
