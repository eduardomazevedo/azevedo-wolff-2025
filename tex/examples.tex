...
The first-order approach fails with a reservation wage of $-\$1,000$, but is valid for all weakly positive reservation wages. This is an illustration of our main point, that the first-order approach is broadly valid in most interesting cases. Moreover, this is representative of a more general phenomenon, and not simply this example. Figures \ref{fig:poisson-log-pp} and \ref{fig:gaussian-cara-pp} consider a different distribution (the discrete Poisson distribution) and a different utility function (CARA). The same pattern holds, as it does in a multitude of other examples.
...
As a corollary to Theorem \ref{thm:main}, the problem is often tractable, following standard first-order approach arguments dating back to \textcite{holmstrom1978incentives}. An optimal contract exists, is unique, and optimal wages have a simple formula:
\begin{equation}
    \label{eq:optimal-wage}
    w(y) = k \circ g\biggl(\mu + \lambda S(y | a_0)\biggr) \text{.}
\end{equation}

The function $k \circ g$ is determined by the utility function and the limited liability constraint. $S(y | a_0)$ is the score function (also known as the likelihood ratio), determined by the distribution of output. The optimal action is $a_0$, and $\lambda$ and $\mu$ are Lagrange multipliers. Section XXX gives closed-form solutions for these functions for a wide range of examples, and Section XXX gives numerical methods based on the formulae.
