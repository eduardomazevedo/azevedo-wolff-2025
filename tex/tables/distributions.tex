\begin{table}[ht]
    ...
    \begin{tabular}{l p{5.5cm} p{3.8cm} p{3.8cm}} % Keep all columns aligned and tight
        \toprule
        \textbf{Distribution} & \textbf{Probability Density or Mass Function \(f(y\mid a)\)} & \textbf{Action Score \(\partial_a\log f(y\mid a)\)} & \textbf{Mean} \\
        \midrule
        Gaussian
        & \( \mathcal{N}(a, \sigma^2) \)
        & \( \frac{y - a}{\sigma^2} \) 
    ...
    \caption*{\textit{Note:} The table reports the probability density function (or probability mass function for discrete distributions), action score, and mean as functions of the agent's action $a$.  The score used in the body of the paper is obtained by evaluating the listed action score at the intended action: $s(y)=\partial_a\log f(y\mid a_0)$.}
\end{table}
