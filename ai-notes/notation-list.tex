...
\begin{enumerate}
    ...
    $a_0$.
    \item Compound-modifier conventions are fixed throughout: ``first-order
    approach'' and ``cost-minimization problem'' are hyphenated;
    ``cost minimization'' as a standalone noun phrase is not.  We write
    ``nonlocal'' and ``nonlinear'' as single words.
    \item A vertical bar is reserved for conditioning, as in $f(y\mid a)$.
    Parameters of contracts are therefore subscripts, as in
    $w_{\lambda,\mu}$.
    \item The semantic command $\Revenue(a)$ denotes the principal's expected
    revenue.  The single letter $R$ is not used for this primitive.
\end{enumerate}
...
\Revenue(a) & Principal's expected revenue when action $a$ is taken. \\
\beta & Revenue slope in the linear numerical specification
$\Revenue(a)=\beta a$. \\
\kappa & Effort-cost coefficient used in the numerical experiments. \\
...
C^*(\bar U) & Optimal expected compensation cost in the relaxed problem at
reservation utility $\bar U$, with the fixed intended action $a_0$ suppressed. \\
...
\begin{itemize}
    ...
    proximity to the threshold $m_0$;
    \item reserved $\rho$, $\theta$, and $q$ for their proof roles by using
    $\beta$, $\kappa$, and explicit conditional means in the numerical
    specification; and
...
likelihood; do not use $\ell$ for anything other than the utility link function;
do not use $R$ in place of $\Revenue$; and do not revert to $u(w)$ or $u(x)$
when utility is over final wealth.

\section{Cross-reference style}

Capitalize numbered formal elements and structural divisions: Theorem,
Proposition, Lemma, Corollary, Remark, Condition, Assumption, Definition,
Section, Appendix, Figure, Table, and Algorithm.  Use lowercase
``equation'' except at the beginning of a sentence, and use lowercase
``part'' except at the beginning of a sentence.  Insert a nonbreaking space
between a reference type and its number, for example
\verb|Theorem~\ref{thm:main}|.  Refer to equations with \verb|\eqref{...}|,
either alone or after ``equation,'' rather than with \verb|(\ref{...})|.
