...
Let $\underline c'' := \inf_{a\in\cA}c''(a)$.
...
\begin{assumption}[Canonical-contract regularity]
\label{ass:contract-regularity}
Every canonical utility contract $v_{\lambda,\mu}$ belongs to $\mathcal V$.
Moreover, its expected utility, local incentives, and expected compensation cost vary
continuously with the contract parameters: the maps
\[
 (\lambda,\mu)\longmapsto
 U(v_{\lambda,\mu},a_0),
 \qquad
 (\lambda,\mu)\longmapsto
 U_a(v_{\lambda,\mu},a_0),
 \qquad
 (\lambda,\mu)\longmapsto
 C(v_{\lambda,\mu},a_0)
\]
are continuous on $\R^2$.
\end{assumption}

\begin{remark}[Domination as a sufficient condition]
Assumption~\ref{ass:contract-regularity} states directly the regularity used in
the arguments.  Recall that membership in $\mathcal V$ already requires finite
expected compensation and valid differentiation under the integral sign through
second order.  The additional continuity in $(\lambda,\mu)$ and membership in
$\mathcal V$ can be verified together by standard local domination conditions.  Distributional
examples in the appendix should verify such primitive conditions rather than
build them into the statement of the main results.
\end{remark}
...
criterion used below.  Suppose Assumption~\ref{ass:link-concavity} and
\eqref{eq:cost-curv} hold.  If there are $q_0<q_1\le0$ in
...
Suppose Proposition~\ref{prop:relaxed-solution},
Assumption~\ref{ass:link-concavity}, and \eqref{eq:cost-curv} apply.  Suppose,
