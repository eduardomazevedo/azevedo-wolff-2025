...
on $[\underline u,\bar u)$.  A utility contract therefore has associated
final-wealth and payment contracts
\[
 W(y)=k(v(y)),
 \qquad
 w(y)=k(v(y))-w_0.
...
$c'(a_0)>0$.  The agent's \textbf{expected utility} and the principal's
\textbf{expected compensation cost} are
\begin{align*}
 U(v,a)&:=\int v(y)f(y\mid a)\dd(y)-c(a),\\
 C(v,a)&:=\int [k(v(y))-w_0]f(y\mid a)\dd(y).
\end{align*}
The first argument of $U$ and $C$ is always a utility contract.  We do not
overload these functionals by writing $U(w,a)$ or $U(W,a)$; payment and
...
\begin{definition}[Admissible utility contracts]
A utility contract $v:\cY\to[\underline u,\bar u)$ is \textbf{admissible} if it
is measurable, has finite expected compensation cost $C(v,a)$ for every
$a\in\cA$, and the map
\[
...
\end{equation}
The \textbf{feasible contract set} $\mathcal V$ is the set of all admissible
utility contracts.
\end{definition}
...
Moreover, the following maps are well defined and continuous on $\R^2$:
\begin{align*}
 (\lambda,\mu)&\longmapsto
 \int \ell(\lambda+\mu s(y))s(y)f_0(y)\dd(y),\\
 (\lambda,\mu)&\longmapsto
 \int \ell(\lambda+\mu s(y))f_0(y)\dd(y),\\
 (\lambda,\mu)&\longmapsto
 \int [L(\lambda+\mu s(y))-w_0]f_0(y)\dd(y).
\end{align*}
\end{assumption}
