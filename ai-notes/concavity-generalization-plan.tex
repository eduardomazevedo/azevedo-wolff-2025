...
\begin{enumerate}
 ...
 \item \textbf{Scope and endogenous intended actions.}  State clearly that the
 main theorem is action-by-action for a fixed interior $a_0$.  A theorem for the
 principal's problem with endogenous action requires uniformity over relevant
 intended actions and separate treatment of boundary actions and action-dependent
 upper utility frontiers.  Include such a result only if those uniform conditions
 can be stated and proved cleanly; otherwise discuss it as an extension rather
 than obscuring the main theorem.
\end{enumerate}
