...
\todo{State the minimal condition needed for strict Pareto-path comparative
statics.  With discrete scores and binding limited liability, the canonical path
may have flat portions even though local implementability holds.  We should
weaken or localize the strict-monotonicity claims if possible rather than impose
a nondegeneracy condition that excludes the discrete examples.}

\begin{proposition}[Generalized relaxed solution]
\label{prop:relaxed-solution}
Subject to resolution of the preceding nondegeneracy open point:
...
everywhere unique solution
\[
 v^*_{\bar U}(y)
 =\ell\bigl(\lambda^*(\bar U)+\mu^*(\bar U)s(y)\bigr)
\]
characterized by Proposition~\ref{prop:relaxed-solution}.  If $\bar u=\infty$,
...
$\lambda_0$.  Proposition~\ref{prop:relaxed-solution} gives
$\lambda^*(\bar U)\to\infty$ as $\bar U\uparrow U_R$.  Choose $U^*$ so that
$\lambda^*(\bar U)\ge\lambda_0$ above $U^*$.  Concavity and LIC at the interior
action $a_0$ give global incentive compatibility.  Thus the unique relaxed
solution is feasible for, and uniquely solves, the original problem.
